...
>(({ className, children, onInteractOutside, onEscapeKeyDown, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 grid w-full gap-4 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 overflow-y-auto",
        // Mobile: fullscreen
        "inset-0 max-h-screen p-3 sm:p-6",
        // Desktop: centralizado com max-width
        "sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:max-w-lg sm:max-h-[95vh] sm:rounded-lg",
        // Animações
