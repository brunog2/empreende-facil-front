    ...
    <div className="mx-auto max-w-6xl space-y-6 p-4 md:p-6">
      ...
      <Card className="overflow-hidden border-primary/20">
        ...
        <CardContent className="grid gap-4 border-t bg-muted/30 pt-6 sm:grid-cols-3">
          ...
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Próxima renovação</p><p className="mt-1 font-medium">{formatDate(renewal)}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Teste gratuito</p><p className="mt-1 font-medium">{subscription.status === "trialing" ? `${trialDays ?? 0} dias restantes` : "Concluído"}</p></div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
        ...
        <Card><CardHeader><CardTitle>Ações</CardTitle><CardDescription>Gerencie a continuidade da assinatura.</CardDescription></CardHeader><CardContent className="space-y-3">
          <Button className="w-full" onClick={() => navigate("/planos")}>Alterar plano</Button>
          ...
          {subscription.cancelAtPeriodEnd ? <Button className="w-full" variant="outline" disabled={reactivate.isPending} onClick={() => run(() => reactivate.mutateAsync(), "Cancelamento desfeito.")}><RefreshCcw className="mr-2 h-4 w-4" />Reativar assinatura</Button> : <Button className="w-full" variant="ghost" disabled={cancel.isPending} onClick={() => run(() => cancel.mutateAsync(), "Assinatura será cancelada ao fim do período.")}>Cancelar no fim do período</Button>}
