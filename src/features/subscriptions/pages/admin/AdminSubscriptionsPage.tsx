...
export default function AdminSubscriptionsPage() {
  ...
  const cards = metrics.data ? [
    ...
    { title: "Em teste", value: metrics.data.trialing, icon: CalendarPlus },
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div><h1 className="font-display text-3xl font-bold">Assinaturas</h1><p className="text-muted-foreground">Acompanhe contratos, testes, receita estimada e a saúde da base.</p></div>
      ...
      <Card><CardHeader><CardTitle>Contas e assinaturas</CardTitle><div className="grid gap-3 pt-3 md:grid-cols-3"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Nome, e-mail ou negócio" value={search} onChange={(e)=>{setSearch(e.target.value);setPage(1);}} /></div><Select value={status} onValueChange={(value)=>{setStatus(value);setPage(1);}}><SelectTrigger><SelectValue placeholder="Status" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os status</SelectItem>{(["trialing","active","past_due","suspended","canceled","expired"] as SubscriptionStatus[]).map((value)=><SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select><Select value={plan} onValueChange={(value)=>{setPlan(value);setPage(1);}}><SelectTrigger><SelectValue placeholder="Plano" /></SelectTrigger><SelectContent><SelectItem value="all">Todos os planos</SelectItem>{plans.data?.map((item)=><SelectItem key={item.id} value={item.code}>{item.name}</SelectItem>)}</SelectContent></Select></div></CardHeader><CardContent>
        {subscriptions.isLoading ? <div className="space-y-2">{[1,2,3,4].map((i)=><Skeleton key={i} className="h-14" />)}</div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Responsável</TableHead><TableHead>Negócio</TableHead><TableHead>Plano</TableHead><TableHead>Status</TableHead><TableHead>Ciclo</TableHead><TableHead>Renovação / teste</TableHead><TableHead>Último pagamento</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{subscriptions.data?.data.map((item)=><TableRow key={item.id}><TableCell><p className="font-medium">{item.user?.fullName ?? "Conta excluída"}</p><p className="text-xs text-muted-foreground">{item.user?.email ?? "Histórico preservado"}</p></TableCell><TableCell>{item.user?.businessName ?? "—"}</TableCell><TableCell>{item.plan.name}</TableCell><TableCell><SubscriptionStatusBadge status={item.status} /></TableCell><TableCell>{item.billingCycle === "yearly" ? "Anual" : "Mensal"}</TableCell><TableCell>{formatDate(item.currentPeriodEnd ?? item.trialEndsAt)}</TableCell><TableCell>{formatDate(item.lastPayment?.paidAt ?? null)}</TableCell><TableCell><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" title="Ver detalhes" onClick={()=>setSelected(item)}><Eye className="h-4 w-4" /></Button><Button size="icon" variant="ghost" title="Estender teste em 7 dias" onClick={()=>execute(()=>actions.extendTrial.mutateAsync({id:item.id,days:7}),"Teste estendido em 7 dias.")}><CalendarPlus className="h-4 w-4" /></Button>{item.status === "suspended" ? <Button size="icon" variant="ghost" title="Reativar" onClick={()=>execute(()=>actions.reactivate.mutateAsync(item.id),"Assinatura reativada.")}><PlayCircle className="h-4 w-4" /></Button> : <Button size="icon" variant="ghost" title="Suspender" onClick={()=>execute(()=>actions.suspend.mutateAsync(item.id),"Assinatura suspensa.")}><PauseCircle className="h-4 w-4" /></Button>}</div></TableCell></TableRow>)}</TableBody></Table></div>}
